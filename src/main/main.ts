...
ipcMain.on('print', (e, url) => {
  workerWindow?.webContents.send('print', url);
});

ipcMain.on('readyToPrint', () => {
  workerWindow?.webContents.printToPDF({}).then(data => fs.writeFileSync('QR.pdf', data));
  workerWindow?.webContents.print({}, (success, failureReason) => {
